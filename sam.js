(function() {
    ...
    m.id = MENU_ID;
    m.style = 'position:fixed;top:5%;left:55%;z-index:99999;width:400px;height:600px;min-width:300px;background:#fff;border:2px solid #004589;padding:0;font-family:sans-serif;box-shadow:0 8px 30px rgba(0,0,0,0.5);border-radius:10px;overflow:hidden;display:flex;flex-direction:column;resize:both;';
    ...
    b.style = 'padding:10px;overflow:hidden;flex-grow:1;display:flex;flex-direction:column;background:#f4f4f4;';
    m.appendChild(b);
    ...
    const fillForm = (type) => {
        ...
        setTimeout(() => {
            if (typeof grabar === 'function') grabar();
        }, 150);
    };
    ...
    const renderPatientScanner = (data) => {
        ...
        b.innerHTML = `
            ...
            <div style="flex-shrink:0;margin-top:10px;border-top:2px solid #ccc;padding-top:10px">
                ...
                <select id="menuSel" style="width:100%;margin:5px 0;padding:5px"><option value="mon">1. Monitorizaciones</option></select>
    ...
    const fetchPatients = async (serviceId, mapId) => {
        b.innerHTML = '<div style="text-align:center;margin-top:20px;"><small>Cargando lista de pacientes...</small></div>';
        try {
            const r = await fetch('http://10.7.33.28/hlcm6/atehos002.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'ser=' + serviceId });
            const text = await r.text();
            const p = new DOMParser().parseFromString(text, 'text/html');
        ...
        } catch (e) { alert('Error: ' + e); initAssistant(); }
    };
